...
class ArticleSlideContractTests(unittest.TestCase):
    def test_skill_documents_structured_article_mode(self):
        ...
        for marker in ("article mode", "claim", "equation", "theorem", "speaker notes"):
    ...
    def test_html_has_notes_surface_and_article_quality_checks(self):
        text = HTML.read_text(encoding="utf-8")
        self.assertIn("speakerNotes", text)
    ...
    def test_article_fixture_has_narrative_components(self):
        ...
        self.assertTrue(any(slide.get("speakerNotes") for slide in data["slides"]))
